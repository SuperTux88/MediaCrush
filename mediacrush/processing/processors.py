...
class VideoProcessor(Processor):
        ...
        for s in self.processor_state['streams']:
            if 'info' in s:
                if 'video_codec' in s['info'] and s['info']['video_codec'] == 'vp8':
                    skip_webm = True
        if not skip_webm:
            self._execute("ffmpeg -y -i {0} -c:v libvpx -c:a libvorbis -q:a 5 -pix_fmt yuv420p -quality good -b:v 2M -crf 5 -vf " + filter_string + map_string + " {1}.webm")
    ...
    def async(self):
        ...
        if self.processor_state['has_audio']:
            map_string += ' -map 0:a:0'
        self._execute("ffmpeg -y -i {0} -q:v 3 -q:a 4 -pix_fmt yuv420p -acodec libvorbis -vcodec libtheora" + map_string + " {1}.ogv")
...
class AudioProcessor(Processor):
    ...
    def sync(self):
        self._execute(copy)
        self._execute("ffmpeg -y -i {0} -acodec libmp3lame -q:a 5 -map 0:a:0 {1}.mp3")
